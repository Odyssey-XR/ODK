...
using ODK.Roslyn.Netcode.Abstract;
...
namespace ODK.Roslyn.Netcode.Analysers
{
  ...
  public class NetworkMethodAnalyzer : DiagnosticAnalyzer
  {
    ...
    private static bool IsNetworkMethod(IMethodSymbol methodSymbol)
    {
      ...
      return IsNetworkBehaviourType(containingType);
    }

    private static bool IsNetworkBehaviourType(INamedTypeSymbol? typeSymbol)
    {
      if (typeSymbol is null)
        return false;

      if (typeSymbol.Name == "NetworkBehaviour" && typeSymbol.ContainingNamespace.ToString() == "Unity.Netcode")
        return true;

      foreach (INamedTypeSymbol? @interface in typeSymbol.Interfaces)
      {
        if (IsNetworkBehaviourType(@interface))
          return true;
      }

      return IsNetworkBehaviourType(typeSymbol.BaseType);
    }
